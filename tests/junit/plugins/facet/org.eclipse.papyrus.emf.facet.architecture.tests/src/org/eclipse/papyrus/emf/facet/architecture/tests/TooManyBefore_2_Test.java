...
@PluginResource("resources/TooManyBefore/TooManyBefore_2_Test.architecture")
public class TooManyBefore_2_Test extends AbstractTest {

	@Test
	public void test() {
		final List<Diagnostic> diagnostics = validate();
		Assert.assertEquals("Diagnostic not found", 3, diagnostics.size()); //$NON-NLS-1$
		int nbTooManyBeforeForMe = 0;
		int nbExtraBefore = 0;
		for (final Diagnostic diag : diagnostics) {
			...
			if (CustomizationMergeErrorCode.CHECK_ERROR__TOO_MANY_BEFORE_FOR_ME == diag.getCode()) {
				nbTooManyBeforeForMe++;
			}
			if (CustomizationMergeErrorCode.CHECK_ERROR__EXTRA_BEFORE == diag.getCode()) {
				nbExtraBefore++;
			}
		}
		Assert.assertEquals("I don't found the expected error TOO_MANY_REDEFINES_FOR_ME", 1, nbTooManyBeforeForMe); //$NON-NLS-1$
		Assert.assertEquals("I don't found the expected error EXTRA_REDEFINES", 2, nbExtraBefore); //$NON-NLS-1$
	}
