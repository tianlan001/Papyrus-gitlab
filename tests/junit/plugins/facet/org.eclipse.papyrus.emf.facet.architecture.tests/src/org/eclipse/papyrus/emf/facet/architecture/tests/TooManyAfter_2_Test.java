...
@PluginResource("resources/TooManyAfter/TooManyAfter_2_Test.architecture")
public class TooManyAfter_2_Test extends AbstractTest {

	@Test
	public void test() {
		final List<Diagnostic> diagnostics = validate();
		Assert.assertEquals("Diagnostic not found", 3, diagnostics.size()); //$NON-NLS-1$
		int nbTooManyAfterForMe = 0;
		int nbExtraAfter = 0;
		for (final Diagnostic diag : diagnostics) {
			...
			if (CustomizationMergeErrorCode.CHECK_ERROR__TOO_MANY_AFTER_FOR_ME == diag.getCode()) {
				nbTooManyAfterForMe++;
			}
			if (CustomizationMergeErrorCode.CHECK_ERROR__EXTRA_AFTER == diag.getCode()) {
				nbExtraAfter++;
			}
		}
		Assert.assertEquals("I don't found the expected error TOO_MANY_REDEFINES_FOR_ME", 1, nbTooManyAfterForMe); //$NON-NLS-1$
		Assert.assertEquals("I don't found the expected error EXTRA_REDEFINES", 2, nbExtraAfter); //$NON-NLS-1$
	}
