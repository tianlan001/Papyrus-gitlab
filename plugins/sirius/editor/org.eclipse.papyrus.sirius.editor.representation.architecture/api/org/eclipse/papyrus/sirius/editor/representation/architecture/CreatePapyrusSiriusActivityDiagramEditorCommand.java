...
/**
 * This class allows to create new Activity Sirius Diagram instance and open the Papyrus editor for it
 */
public class CreatePapyrusSiriusActivityDiagramEditorCommand extends AbstractCreateSiriusDiagramEditorCommand implements ICreateSiriusDiagramEditorCommand {
	...
	public DSemanticDiagram execute(final ViewPrototype prototype, final String name, EObject semanticContext, final EObject graphicalContext, boolean openAfterCreation) {
		if (prototype instanceof SiriusDiagramViewPrototype) {
			...
			if (semanticContext instanceof NamedElement && !(semanticContext instanceof Activity)) {
				...
				activity.setName(elementDefaultNameProvider.getDefaultName(activity, semanticContext));
				try {
					...
					if (namedElement instanceof Package) {
						ted.getCommandStack().execute(new RecordingCommand(ted) {
							...
							protected void doExecute() {
								...
								((Package) namedElement).getPackagedElements().add(activity);
							}
						});
						...
						ted.getCommandStack().execute(new RecordingCommand(ted) {
							...
							@Override
							protected void doExecute() {
								((BehavioredClassifier) namedElement).getOwnedBehaviors().add(activity);
								((BehavioredClassifier) namedElement).setClassifierBehavior(activity);
							}
						});
					}
