 ...
 */
public class InputContentPapyrusReferenceLifecycleManagerProvider implements IEEFLifecycleManagerProvider {
	...
	public IEEFLifecycleManager getLifecycleManager(EEFControlDescription controlDescription,
		...
		if (!(controlDescription instanceof EEFInputContentPapyrusReferenceDescription)
				|| Util.isBlank(((EEFInputContentPapyrusReferenceDescription) controlDescription).getInputContentExpression())) {
			return null;
		}
		...
		String inputContentExpression = Optional.ofNullable(description.getInputContentExpression()).orElse(""); //$NON-NLS-1$
		String referenceName = this.getReferenceName(description, interpreter, variableManager);

		if (target != null && !Util.isBlank(inputContentExpression) && !Util.isBlank(referenceName)) {
